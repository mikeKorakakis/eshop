...
CREATE TABLE IF NOT EXISTS `users` (
  ...
  `trust_status` INT(11) NOT NULL DEFAULT 0 COMMENT 'Trust level or rank', -- to be deleted
  `registration_status` TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'Registration approval status', -- to be deleted
...
CREATE TABLE IF NOT EXISTS `categories` (
  ...
  `is_visible` TINYINT(1) NOT NULL DEFAULT 1 COMMENT 'Visibility status', -- to be deleted
  `allow_comments` TINYINT(1) NOT NULL DEFAULT 1 COMMENT 'Allow comments in category', -- to be deleted
  `allow_ads` TINYINT(1) NOT NULL DEFAULT 1 COMMENT 'Allow ads in category', -- to be deleted
  PRIMARY KEY (`category_id`),
  UNIQUE KEY `name` (`name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- Table: `items`
CREATE TABLE IF NOT EXISTS `items` ( -- to be renamed to `products`
  `item_id` INT(11) NOT NULL AUTO_INCREMENT COMMENT 'Unique ID for item', -- to be renamed to `product_id`
  ...
  `status` VARCHAR(50) NOT NULL COMMENT 'Status (e.g., Available, Sold)', -- to be deleted
  `rating` SMALLINT(6) NOT NULL DEFAULT 0 COMMENT 'Rating of the item', -- to be deleted
  `is_approved` TINYINT(1) NOT NULL DEFAULT 1 COMMENT 'Approval status', -- to be deleted
  `category_id` INT(11) NOT NULL COMMENT 'Category ID', 
  `owner_id` INT(11) NOT NULL COMMENT 'Owner/User ID', 
  `image_url` VARCHAR(255) NOT NULL COMMENT 'URL to item image',
  `contact_info` VARCHAR(255) NOT NULL COMMENT 'Contact information', -- to be deleted
  PRIMARY KEY (`item_id`), -- to be renamed to `product_id`
...
CREATE TABLE IF NOT EXISTS `comments` (
  ...
  `status` TINYINT(1) NOT NULL COMMENT 'Approval status of comment', -- to be deleted
