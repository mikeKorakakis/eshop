...
import { updateCustomerPasswordMutation } from '@/lib/vendure/shop/customer/customer';
...
export default function ChangePasswordView({ dictionary }: Props) {
  const common_dictionary = dictionary.common;
  const profile_dictionary = dictionary.profile;
  const [loading, setLoading] = useState(false);
  const changePassword = async (data: ChangePasswordType) =>
    await updateCustomerPasswordMutation(data.currentPassword, data.newPassword);
  ...
  const onSubmit = async (data: ChangePasswordType) => {
    try {
      ...
      console.log(err);
      if (err == 'Error: InvalidCredentialsError') {
        toast.error(profile_dictionary.password_invalid);
      } else {
