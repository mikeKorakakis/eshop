				...
				<div className="bg-gray-900">
					<div className="mx-auto flex h-10 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
						<span className="mr-4 flex text-sm font-medium text-white hover:text-gray-100">
							<a href={process.env.NEXT_PUBLIC_OLD_URL}><a>{common_dictionary.visti_old_site}</a></a>
						</span>
						{/* Currency selector */}
						<div></div>
									...
									<div className="hidden h-full space-x-4 lg:flex">
										...
										{!isAdmin && (
											<Link
												href="/"
												className={clsx(
													'flex items-center border-red-500 text-sm font-medium hover:border-b',
													isRoot ? 'border-b-2 border-red-500 text-white' : 'text-black'
												)}
											>
														...
														{({ open }) => (
															<>
																<div className="relative flex">
																	<Popover.Button
																		className={clsx(
																			'relative z-10 flex items-center justify-center border-red-500 text-sm font-medium transition-colors duration-200 ease-out hover:border-b',
																			isRoot ? 'text-white' : 'text-black',
																			testPath.includes(navigation.categories?.tag) &&
