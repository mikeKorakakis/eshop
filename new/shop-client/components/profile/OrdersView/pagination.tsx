      ...
      <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
        <div>
          <p className="text-sm text-gray-700">
            ...
            <span className="font-medium">{totalItems > 0 ? skip + 1 : 0}</span> {common_dictionary.to}{' '}
            <span className="font-medium">
              {skip + (take ?? pageSize) > totalItems ? totalItems : skip + (itemCount ?? pageSize)}
            </span>{' '}
            {common_dictionary.of} <span className="font-medium">{totalItems}</span> {common_dictionary.results}
          </p>
        </div>
        <div>
          <nav
              ...
              if (totalPages > 0 && totalPages < 10) {
                return (
                  <button
                    ...
                    aria-current="page"
                    className={clsx(
                      'z-pageSize relative inline-flex items-center border px-4 py-2 text-sm font-medium  ',
                      currentPage === i + 1
                        ? ' z-20 border-red-500  bg-red-50 text-red-600'
                ...
                if (currentPage < 4 || currentPage > totalPages - 3) {
                  ...
                  return (
                    <Fragment key={i}>
                      ...
                      {(i === currentPage || i === currentPage - 1 || i === currentPage - 2) && (
                        <button
                          type="button"
                          onClick={() => setSkip(i * (take ?? pageSize))}
                          aria-current="page"
                      ...
                      {i === totalPages - 3 && i !== currentPage && currentPage >= 4 && (
                        <span className="relative inline-flex items-center border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700">
                          ... {i === totalPages - 3} {i !== currentPage + 2}
                        </span>
                      )}
                      {i === totalPages - 4 && i !== currentPage && currentPage < 4 && (
                        <span className="relative inline-flex items-center border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700">
                          ... {i === totalPages - 3} {i !== currentPage + 2}
                        </span>
                      )}
                      {(i === totalPages - 1 ||
                        ...
                        <button
                          ...
                          aria-current="page"
                          className={clsx(
                            'z-pageSize relative inline-flex items-center border px-4 py-2 text-sm font-medium  focus:z-20',
                            currentPage === i + 1
